...
public bool CallFireSupport(int client, float ground[3]) {									// HINT: Fire to target pos
	...
	if (GetSkyPos(client, ground, sky))
	{
		...
		g_flFireSupportTimer = gCvarDelay.FloatValue;
		...
		ShowDelayEffect(ground, sky, g_flFireSupportTimer);

		CreateTimer(1.0, Timer_CountDown, client, TIMER_FLAG_NO_MAPCHANGE);
		CreateTimer(g_flFireSupportTimer + 0.05 + GetURandomFloat(), Timer_LaunchMissile, pack, TIMER_FLAG_NO_MAPCHANGE);
		CreateTimer(g_flFireSupportTimer + 0.05 + 1.05 * shells, Timer_DataPackExpire, pack, TIMER_FLAG_NO_MAPCHANGE | TIMER_DATA_HNDL_CLOSE);
		return true;
	}
